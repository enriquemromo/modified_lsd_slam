...
#include "lsd_slam_viewer/PointCloudArrayMsg.h"
#include "lsd_slam_viewer/PointCloudMsg.h"
...
void rosThreadLoop( int argc, char** argv )
{
	...
	ros::Publisher pointcloud_pub = nh.advertise<lsd_slam_viewer::PointCloudArrayMsg>("lsd_slam/pointcloud", 1000);
	...
	while (ros::ok())
	{
		...
		lsd_slam_viewer::PointCloudArrayMsg pointCloudArrayMsg;
		
		std::vector<Point> pointCloud;

		if(viewer != 0){
			pointCloud = viewer->getPoints();
		}
	...
	{
		lsd_slam_viewer::PointCloudMsg pointCloudMsg;
		
		pointCloudMsg.x =  (*it).x;
		pointCloudMsg.y =  (*it).y;
		pointCloudMsg.z =  (*it).z;
		
		pointCloudArrayMsg.points.push_back(pointCloudMsg);
		
	}
	
		pointcloud_pub.publish(pointCloudArrayMsg);
		if(viewer != 0){
