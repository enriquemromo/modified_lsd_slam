...
void KeyFrameGraphDisplay::draw()
{
	...
	if(sendPointcloud)
	{
		...
		for(unsigned int i=0;i<keyframes.size();i++)
		{
			if((int)i > cutFirstNKf)
			{
				std::vector<Point> keyframePoinst = keyframes[i]->getPoints();
				this->pointcloud.insert(this->pointcloud.end(), keyframePoinst.begin(),
				keyframePoinst.end());
			}
		}
...
//	printf("graph update: %d constraints, %d poses\n", msg->numConstraints, msg->numFrames);
	...
	std::vector<Point> KeyFrameGraphDisplay::getPointcloud()
	{
	
		return this->pointcloud;
	}

	void KeyFrameGraphDisplay::clearPointcloud()
	{
